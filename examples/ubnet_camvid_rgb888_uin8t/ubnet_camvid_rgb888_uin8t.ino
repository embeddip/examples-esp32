/**
 * CamVid RGB888 segmentation on ESP32 with embedDIP + TFLite Micro
 */
...
void setup() {
  ...
  bool img_ok = true;
  img_ok &= (createImageWH(INPUT_WIDTH, INPUT_HEIGHT, IMAGE_FORMAT_RGB888, &inImg) == EMBEDDIP_OK);
  img_ok &= (createImageWH(INPUT_WIDTH, INPUT_HEIGHT, IMAGE_FORMAT_GRAYSCALE, &grayImg) == EMBEDDIP_OK);
  img_ok &= (createImageWH(INPUT_WIDTH, INPUT_HEIGHT, IMAGE_FORMAT_RGB888, &visImg) == EMBEDDIP_OK);
  img_ok &= (createImageWH(INPUT_WIDTH, INPUT_HEIGHT, IMAGE_FORMAT_RGB888, &overlayImg) == EMBEDDIP_OK);

  if (!img_ok || !inImg || !grayImg || !visImg || !overlayImg) {
    return;
  }
...
void loop() {
  ...
  if (digitalRead(PIN_BUTTON) == LOW) {
    if (serial->capture(inImg) != EMBEDDIP_OK) {
      delay(100);
      return;
    }

    if (!prepareModelInput(inImg, input_tensor)) {
      delay(100);
      return;
    }
    ...
    if (!processModelOutput(output_tensor)) {
      delay(100);
      return;
    }
